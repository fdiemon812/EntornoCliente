...
//
//   const searchRESTCountries = Convert.toSearchRESTCountries(json);
//
// These functions will throw an error if the JSON doesn't
// match the expected interface, even if the JSON is valid.

export interface SearchRESTCountries {
    name:         Name;
    tld:          string[];
    cca2:         string;
    ccn3:         string;
...
export class Convert {
    public static toSearchRESTCountries(json: string): SearchRESTCountries[] {
        return cast(JSON.parse(json), a(r("SearchRESTCountries")));
    }

    public static searchRESTCountriesToJson(value: SearchRESTCountries[]): string {
        return JSON.stringify(uncast(value, a(r("SearchRESTCountries"))), null, 2);
    }
}
